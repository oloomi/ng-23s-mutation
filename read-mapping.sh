...
bowtie2-build -q ${ref_genome} ${out_dir}genome-index

# Single mapping, best-match
bowtie2 --quiet -x ${out_dir}genome-index -U ${reads_out_dir}${reads_file_prefix}_1.fq,${reads_out_dir}${reads_file_prefix}_2.fq \
-S ${out_dir}${file_prefix}-mapping-best-match.sam 2> ${out_dir}${file_prefix}-mapping-best-match-log.txt

# Single mapping, report-all
bowtie2 --quiet -a -x ${out_dir}genome-index -U ${reads_out_dir}${reads_file_prefix}_1.fq,${reads_out_dir}${reads_file_prefix}_2.fq \
...
bowtie2-build -q ${ref_genome} ${out_dir}masked-genome-index

# Single mapping, best-match, masked genome
bowtie2 --quiet -x ${out_dir}masked-genome-index -U ${reads_out_dir}${reads_file_prefix}_1.fq,${reads_out_dir}${reads_file_prefix}_2.fq \
